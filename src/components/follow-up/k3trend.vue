  ...
  <div class="lottery_box" v-if="show">
    <div class="followup_container">
        <div class="top_header">
            <div class="empty"></div>
            <div class="word">k3 Lotre ( {{ gameTypes[currentTime].label }} {{ $t("lottery.minute")}} )</div>
            <div class="icon">
        ...
        <div class="container">
                <div class="container_c">
                    ...
                    <div class="canvans_box">
                        <canvas  ref="myCanvas2"  :style="{width: '304px', height: '150px'}"></canvas >
                    </div>
                </div>
                <div class="container_c">
                    <div class="explain_box">
                        <div class="top_filter">
                            <div class="label">{{ $t('lottery.k3odd') }}:</div>
                            <div class="value odd">O</div>
                        </div>  
                        <div class="top_filter">
                            <div class="label">{{ $t('lottery.k3even') }}:</div>
                            <div class="value even">E</div>
                        </div>    
                    </div>
                    <div class="canvans_box">
                        <canvas  ref="myCanvas3"  :style="{width: '304px', height: '150px'}"></canvas >
                    </div>
                </div>
        </div>
         <van-overlay
...
const ctx3 = ref(null)
const renderVal3:any= ([])
const satVal3:any = ref([])
const myCanvas2 = ref(null)
const myCanvas3 = ref(null)
const showTrend = async(gameId=4)=> {
    show.value = true
    // console.log(myCanvas2.value)
    nextTick(()=>{
        ctx2.value = myCanvas2.value.getContext('2d')
        ctx3.value = myCanvas3.value.getContext('2d')
        initData(gameId)
    })
}
const initData=(gameId:any)=> {
    ...
    var newArr = [];
    resultRecord({game_id:gameId,pageIndex:1,pageSize:100}).then((res:any)=>{
        arrs.value = res.rows
        
        let arrVal = res.rows.map((item:any)=>{
            let amount = 0
            return item.result.reduce((amount:any,item:any)=>Number(amount)+Number(item),amount)
        })
        ...
        renderVal.value = newArr.slice(0,15);
        // initArr(1)
        // init(1);
        // ctx.value.draw();
        setTimeout(()=>{
...
const init=(type:any)=> {
    // console.log(type,"typeeee",renderVal2.value,renderVal3.value)
    let initArr:any;
    // let initArr = type == 1 ? renderVal.value : renderVal2.value;
    if(type == 1) {
        initArr = renderVal.value
    }else if(type ==2) {
        initArr = renderVal2.value
    }else {
        initArr = renderVal3.value
    }
        ...
        for(var i = 0;i<lessEight.length;i++) {
            if(lessEight[i].length == 1) {
                if(type == 1) {
                    ...
                    renderCanvas3(i,0,lessEight[i][0])
                }
            }else {
                for(var j = 0;j<lessEight[i].length;j++) {
                    if(type ==1) {
                        ...
                        renderCanvas3(i,j,lessEight[i][0]);
                    }
                }
            }
        }
            ...
            if(i == 0) {
                for(var j = 0;j<moreEight[0].length;j++) {
                    ...
                    if(type ==1) {
                        ...
                        renderCanvas3(x,y,moreEight[0][0]);
                    }
                    ...
                    if(sats[cupiedIndex] && sats[cupiedIndex].copuied) {
                        ...
                        for(var k = 0;k<moreNum;k++) {
                            if(type ==1) {
                                ...
                                renderCanvas3(firstX+k > 14 ? 14 : firstX+k ,firstY,moreEight[i][0]);
                            }
                        ...
                        if(findindex == -1) {
                            ...
                            for(var k = 0;k<moreNum;k++) {
                                if(type ==1) {
                                    ...
                                    renderCanvas3(x+k>14?14:x+k,5,moreEight[i][0]);
                                }
                            ...
                            if(type ==1) {
                                ...
                                renderCanvas3(x,y,moreEight[i][0])
                            }
                            sats[findindex].copuied = true
                        }
                    }
...
const initData2 =()=> {
    var newArr = [];
    var sortArr = []
    // let arrVal = arrs.value.map(item=>{
    //     return item.result[0]>4?'B':'S'
    // })
    let arrVal = arrs.value.map((item:any)=>{
        let amount = 0;
        let sum = item.result.reduce((amount:any,item:any)=>Number(amount)+Number(item),amount)
        return sum > 10 ? 'B' : 'S'
    })
...
const initData3 =()=> {
    ...
    renderVal3.value = newArr.slice(0,13).reverse();
    initArr(3)
    init(3);
    // ctx3.value.draw()
}
...
const renderCanvas3=(x:any,y:any,number:any)=> {
    const gcolor = '#418DFC'
    const rcolor = '#EF233B'
    let nummberColor = number == 'O' ? rcolor : gcolor
    const w = 18;
    ctx3.value.beginPath();
    ctx3.value.fillStyle = nummberColor;
    ctx3.value.arc(10+20*x,10+20*y, 8, 0, 2 * Math.PI);
    ctx3.value.fill();

    // 绘制文字
    ctx3.value.font = "12px Arial"
    ctx3.value.fillStyle = '#fff'
    ctx3.value.textAlign = "center"
    ctx3.value.fillText(number, 10+20*x,15+20*y);
}
const closeHistory=()=> {
    show.value = false;
}
