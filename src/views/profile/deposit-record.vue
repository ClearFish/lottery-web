	...
	<div class="bg">
		<div class="top_cont">
			...
			<div class="other_filter">
				<div class="types" @click="showSelest(statusList)">{{itemChosedItem && itemChosedItem.name ? itemChosedItem.name:$t('promotionactivity.ALL')}}</div>
                    ...
                    <van-list
                     ...
                     >
                        <div class="flex-sb item" v-for="(item,index) in list" :key="index">
                            ...
                            <div class="jf number_width">
                                <div :class="'status status_'+item.status">{{getStatus(item.status)}}</div>
                                <div class="num">+{{numberFormat(item.money)}}</div>
                                <div>{{$t('promotionactivity.trading_recordcurrency')}}: {{item.currency_symbol}}</div>
                            </div>
                        </div>
                    </van-list>
                </van-pull-refresh>
		...
		<van-popup :show="showTypes" position="bottom" class="popupStyle" closeOnClickOverlay>
...
import { getEpayRecord } from '@/api/service'
...
const getFundRecord=async()=>{
    // epayRecord(params.value).then(res =>{
    //     this.total = res.data.total
    //     list.value = list.value.concat(res.data.rows)
    //     if (list.value.length===this.total) {
    //         this.status = 'nomore'
    //     }
    // })
    let res:any = await getEpayRecord({pageIndex: 1,...params.value});
    list.value = res.rows
}
