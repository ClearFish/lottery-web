        ...
        <div class="content">
            ...
            <div class="sidebar">
                <div class="category-list">
                    ...
                    >
                        <div class="category-icon" :class="{icon_active:selectedCategory === category.game_type}">
            ...
            <!-- 右侧游戏网格 -->
            <div class="main-content" ref="gamegirdRef">
                 <div class="video_container" v-if="selectedCategory == 'hot'">
                    <VideoCom/>
                </div>
                <!-- 搜索框 -->
...
import VideoCom from "./video-component.vue"
...
import Lwin1 from "@/assets/lottery/win_1.jpg"
import Lwin3 from "@/assets/lottery/win_3.jpg"
import Lwin5 from "@/assets/lottery/win_5.jpg"
import Lwin10 from "@/assets/lottery/win_10.jpg"
import Lk31 from "@/assets/lottery/k3_1.jpg"
import Lk33 from "@/assets/lottery/k3_3.jpg"
import Lk35 from "@/assets/lottery/k3_5.jpg"
import Lk3110 from "@/assets/lottery/k3_10.jpg"
import L5d1 from '@/assets/lottery/5d_1.png';
import L5d3 from '@/assets/lottery/5d_3.png';
import L5d5 from '@/assets/lottery/5d_5.png';
import L5d10 from '@/assets/lottery/5d_10.png';
    ...
    [
        {
            ...
            game_type:'Lottery',
            types:[
                {title:'1 phút',collect:false,group_id:'2',game_type:'Lottery',icon:Lk31,gameIntro:'K3Lotre1m',minType:1,game_list_id:5},
                {title:'3 phút',collect:false,group_id:'2',game_type:'Lottery',icon:Lk33,gameIntro:'K3Lotre3m',minType:3,game_list_id:6},
                {title:'5 phút',collect:false,group_id:'2',game_type:'Lottery',icon:Lk35,gameIntro:'K3Lotre5m',minType:5,game_list_id:7},
                {title:'10 phút',collect:false,group_id:'2',game_type:'Lottery',icon:Lk3110,gameIntro:'K3Lotre10m',minType:10,game_list_id:8},
            ]
        },
        {   
            title:'Win go',
            ...
            game_type:'Lottery',
            types:[
                {title:'1 phút',collect:false,group_id:'0',game_type:'Lottery',icon:Lwin1,gameIntro:'Color1m',minType:1,game_list_id:1},
                {title:'3 phút',collect:false,group_id:'0',game_type:'Lottery',icon:Lwin3,gameIntro:'Color3m',minType:3,game_list_id:2},
                {title:'4 phút',collect:false,group_id:'0',game_type:'Lottery',icon:Lwin5,gameIntro:'Color1m',minType:5,game_list_id:3},
                {title:'10 phút',collect:false,group_id:'0',game_type:'Lottery',icon:Lwin10,gameIntro:'Color1m',minType:10,game_list_id:4},
            ]
        },
        {
            title:'5D Lotre',
            ...
            game_type:'Lottery',
            types:[
                {title:'1 phút',collect:false,group_id:'1',game_type:'Lottery',icon:L5d1,gameIntro:'Lotre5D1m',minType:1,game_list_id:9},
                {title:'3 phút',collect:false,group_id:'1',game_type:'Lottery',icon:L5d3,gameIntro:'Lotre5D3m',minType:3,game_list_id:10},
                {title:'5 phút',collect:false,group_id:'1',game_type:'Lottery',icon:L5d5,gameIntro:'Lotre5D5m',minType:5,game_list_id:11},
                {title:'10 phút',collect:false,group_id:'1',game_type:'Lottery',icon:L5d10,gameIntro:'Lotre5D10m',minType:10,game_list_id:12},
            ]
        }
    ]
)
...
const selectedProviderVal:any = ref({}) 

// 滚动容器引用
const providerScrollContainer = ref<HTMLElement>()
const gamegirdRef = ref<HTMLElement>()
...
const selectCategory = (gameType: string) => {
    ...
    nextTick(() => {
        ...
        if (gamegirdRef.value) {
            gamegirdRef.value.scrollIntoView({
                block:'start',
                behavior: 'smooth'
            })
        }
...
const selectProvider = (provider: any) => {
    selectedProvider.value = provider?.group_id;
    selectedProviderVal.value = provider;
    searchValue.value = '' // 清空搜索
    isSearching.value = false // 重置搜索状态
    page.value = 1
    games.value = []
...
const filteredProviders = computed(() => {
    if (!selectedCategory.value || selectedCategory.value === 'hot' || selectedCategory.value === 'favorite') {
        return []
    }else if(selectedCategory.value == 'Xổ số Việt Nam') {
        // 彩票分类添加系统彩票
        let providers = gameProviders.value.filter(provider => provider.game_type === selectedCategory.value);
        let K3Lottery = gameLists.value[0];
        K3Lottery.types.reverse()
        let otherLottery = gameLists.value.slice(1)
        providers.unshift(K3Lottery)
        const finalProviders = providers.concat(otherLottery)
...
const loadGames = async () => {
    try {
        ...
        if (searchValue.value.trim()) {
            ...
            if(selectedCategory.value == 'Xổ số Việt Nam') {
                // 彩票在所有游戏列表前添加系统k3彩票,wingo 5d 不请求接口
                if(userStore.isLoggedIn) {
                    await getLotteryFavorite()
                }
                if(selectedProvider.value) {
                    // 有厂商（区分wingo k3 5d）
                    if(selectedProviderVal.value.game_type == 'Lottery') {
                        response.rows = selectedProviderVal.value.types
                        
                    }else {
                        response = await getGameList(params)
                    }
                }else {
                    // 没有选择厂商 在所有游戏列表前添加系统k3彩票
                    response = await getGameList(params)
                    let K3Lotterys = gameLists.value[0].types
                    K3Lotterys.reverse().forEach((item:any)=>response.rows.unshift(item))
                }
            }else {
                // 如果有选择厂商，添加厂商筛选
                if (selectedProvider.value) {
...
const playGame = (game: any) => {
    ...
    if(game.game_type == 'Lottery') {
        // 系统游戏
        let findObj = gameLists.value.map((item:any)=>item.types.find((ite:any)=>ite.game_list_id == game.game_list_id)).filter((it:any)=>it)[0];
        
        router.push({
           path: '/lotre',
           query: {
               game_id: findObj.game_list_id,
               game_type:findObj.group_id,
               gameMin:findObj.minType
           }
       })
    }else {
        router.push({
...
const getLotteryFavorite = async()=>{
    let params = {
        game_type:'Lottery',
        pageIndex:1,
        pageSize:12
    }
    let res = await getMyFavoriteGames(params);
    if(res.rows && res.rows.length) {
        let arr = res.rows;
        gameLists.value.map((item:any)=>{
            if(arr.findIndex(ite=>ite.game_list_id == item.game_id) != -1) {
                item.collect = true
            }
            return item
        })
        console.log(gameLists.value,"gameLists.value")
    }
}
// 初始化
onMounted(async () => {
...
.icon_active {
    img {
        animation: ping .76s ease 0s infinite normal none running;
    }
}
@keyframes ping {
    0% {
        transform: translateY(0);
        animation-timing-function: cubic-bezier(.44, .46, .62, 1);
    }
    50% {
        transform: translateY(-.411rem);
        animation-timing-function: cubic-bezier(.43, .43, .54, 1);
    }
    100% {
        transform: translateY(0);
    }
